 ...
 *聊天面板
 */
var ChatSprite = (function (_super) {
    __extends(ChatSprite, _super);
    function ChatSprite(_main) {
        var _this = _super.call(this) || this;
        _this.showStatus = false;
        _this.sendBool = false; //标记是否在发送阶段
        _this.minY = 0; //聊天记录位移的上限
        _this.maxY = 0; //聊天记录位移的下限
        _this.moveVal = 0; //记录触摸滑动位移
        _this.maskHeight = 230; //遮罩层的高度
        _this.par = _main;
        _this.createView(); //构建显示元素
        return _this;
    }
    //构建显示元素
    ChatSprite.prototype.createView = function () {
    ...
    ChatSprite.prototype.showThis = function () {
    ...
    ChatSprite.prototype.removeThis = function () {
        if (this.showStatus) {
    ...
    //背景触摸开始事件
    ChatSprite.prototype.onBeginTouch = function (event) {
    ...
    //背景触摸滑动事件
    ChatSprite.prototype.onMoveTouch = function (event) {
    ...
    //无用素材的点击 
    ChatSprite.prototype.onTapTouch_bak = function (event) {
        //egret.log("无用点击");
    };
    return ChatSprite;
}(egret.Sprite));
__reflect(ChatSprite.prototype, "ChatSprite");
//# sourceMappingURL=ChatSprite.js.map